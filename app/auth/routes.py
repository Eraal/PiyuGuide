...
@auth_bp.route('/api/departments/<int:campus_id>', methods=['GET'])
def get_campus_departments(campus_id):
    """API endpoint to fetch departments for a given campus (for dynamic dropdown)."""
    try:
...
@login_required 
def logout():
    campus_redirect_id = session.get('selected_campus_id')

    if not campus_redirect_id:
        if current_user.role == 'student' and getattr(current_user, 'student', None) and current_user.student.campus_id:
            campus_redirect_id = current_user.student.campus_id
        elif current_user.role == 'office_admin' and getattr(current_user, 'office_admin', None) and getattr(current_user.office_admin, 'office', None) and current_user.office_admin.office.campus_id:
            campus_redirect_id = current_user.office_admin.office.campus_id
        elif getattr(current_user, 'campus_id', None):
            campus_redirect_id = current_user.campus_id

    if campus_redirect_id:
        campus = Campus.query.get(campus_redirect_id)
        campus_redirect_id = campus.id if campus else None
    ...
    # Clear the auth session campus context; the campus landing route will restore it if needed
    session.pop('selected_campus_id', None)

    flash('You have been logged out.', 'success')
    if campus_redirect_id:
        return redirect(url_for('main.campus_landing', campus_id=campus_redirect_id))
    return redirect(url_for('main.campus_selection'))
