...
def announcement():
    ...
    if current_user.role == 'office_admin':
        # Office Admins see announcements targeted to their office, or public announcements from their campus
        campus_id = current_user.office_admin.office.campus_id
        query = query.outerjoin(User, Announcement.author_id == User.id).filter(
            or_(
                Announcement.target_office_id == current_user.office_admin.office_id,
                and_(Announcement.is_public == True, User.campus_id == campus_id)
            )
        )
    elif current_user.role == 'super_admin' and current_user.campus_id:
        # Campus Admins see announcements targeted to any office in their campus, or public announcements from their campus
        campus_id = current_user.campus_id
        query = query.outerjoin(User, Announcement.author_id == User.id).outerjoin(Office, Announcement.target_office_id == Office.id).filter(
            or_(
                Office.campus_id == campus_id,
                and_(Announcement.is_public == True, User.campus_id == campus_id)
            )
        )
    ...
    if current_user.role == 'super_admin' and current_user.campus_id:
        offices = Office.query.filter_by(campus_id=current_user.campus_id).all()
    else:
