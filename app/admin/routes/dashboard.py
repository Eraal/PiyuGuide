...
def dashboard():
    ...
    if campus_filter:
        ...
        total_students = db.session.query(User).join(Student).filter(
            User.role == 'student',
            Student.campus_id == campus_filter
        ).count()
...
def get_dashboard_stats():
    ...
    if campus_filter:
        upcoming_sessions_query = upcoming_sessions_query.filter(CounselingSession.office_id.in_(campus_office_ids))
        
    upcoming_sessions = upcoming_sessions_query.limit(5).all()
