...
def manage_concern_types():
    ...
    # Get all existing concern types for the campus
    if current_user.campus_id:
        concern_types = ConcernType.query.filter_by(campus_id=current_user.campus_id).order_by(ConcernType.name).all()
    else:
    ...
    if request.method == 'POST':
        ...
        if action == 'add':
            ...
            # Check if concern type with same name already exists for this campus
            existing_type = ConcernType.query.filter(
                func.lower(ConcernType.name) == func.lower(name),
                ConcernType.campus_id == current_user.campus_id
            ).first()
            if existing_type:
                flash(f'A concern type with the name "{name}" already exists for your campus.', 'error')
            else:
            ...
            if not concern:
                ...
                # Check if new name conflicts with existing in the same campus (excluding self)
                existing_type = ConcernType.query.filter(
                    func.lower(ConcernType.name) == func.lower(name),
                    ConcernType.campus_id == current_user.campus_id,
