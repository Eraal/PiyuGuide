...
def handle_send_message(data):
    ...
    client_msg_id = data.get('client_msg_id')  # Extract client-side message ID
    ...
    try:
        ...
        emit('message_sent', {'success': True, 'message_id': new_message.id, 'client_msg_id': client_msg_id})
...
def handle_typing(data):
    ...
    avatar_url = None
    if user and getattr(user, 'profile_pic_path', None):
        from flask import url_for
        avatar_url = url_for('static', filename=user.profile_pic_path) + f"?v={user.profile_pic}"
