...
def handle_join_session(data):
    ...
    emit('session_joined', {
        ...
        'started_at': session.started_at.isoformat() + 'Z' if getattr(session, 'started_at', None) else None,
        'ended_at': session.ended_at.isoformat() + 'Z' if getattr(session, 'ended_at', None) else None
    })
...
def handle_join_call(data):
    ...
    emit('call_joined', {
        ...
        'your_role': current_user.role,
        'started_at': (CounselingSession.query.get(session_id).started_at.isoformat() + 'Z'
...
def handle_end_session(data):
    ...
    if current_user.role in ['office_admin', 'super_admin']:
        ...
        emit('error', {'message': 'Only counselors can end the session for all participants. Please use leave session instead.'})
        return
...
@socketio.on('session_chat_message', namespace='/video-counseling')
def handle_session_chat_message(data):
    """Handle in-session chat message (real-time only, no DB persistence)"""
    session_id = data.get('session_id')
...
@socketio.on('session_chat_typing', namespace='/video-counseling')
def handle_session_chat_typing(data):
    """Handle typing indicator for in-session chat"""
    session_id = data.get('session_id')
...
@socketio.on('call_connected', namespace='/video-counseling')
def handle_call_connected(data):
    """Handle actual WebRTC connection establishment to start the timer"""
    session_id = data.get('session_id')
    if not session_id or session_id not in active_sessions:
        return
    ...
    try:
        ...
        if db_session and not db_session.started_at:
            ...
            emit('call_started', {
                'session_id': session_id,
                'started_at': db_session.started_at.isoformat() + 'Z'
            }, room=room_name)
    except Exception:
        db.session.rollback()
        logger.exception("Failed to set call_connected started_at")
