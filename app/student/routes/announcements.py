...
def announcements():
    ...
    # Count the visible result set before pagination so the template can show real stats
    count_q = q.order_by(None)
    total_count = count_q.count()
    recent_cutoff = datetime.utcnow() - timedelta(days=3)
    new_announcements_count = count_q.filter(Announcement.created_at >= recent_cutoff).count()
    public_announcements_count = count_q.filter(Announcement.is_public.is_(True)).count()
    office_announcements_count = count_q.filter(Announcement.is_public.is_(False)).count()
    ...
    # Mark recent announcements (less than 3 days old) for per-item badges
    today = datetime.utcnow()
    for ann in items:
        ann.is_new = (today - ann.created_at).days < 3
    ...
    return render_template(
        ...
        office_announcements_count=office_announcements_count,
