...
class SmartNotificationsManager {
    ...
    showNewNotificationAlert(newCount) {
        // Prefer the modern office toast if available to avoid duplicate legacy alerts
        try {
            const lastSocket = window.__lastOfficeSocketNotificationAt || 0;
            const now = Date.now();
            // If a socket notification arrived very recently, skip showing a poll-based alert
            if (now - lastSocket < 3000) return;
            const title = newCount > 1 ? `${newCount} new notifications` : `${newCount} new notification`;
            const message = '';
            if (typeof window.officeShowToast === 'function') {
                window.officeShowToast({ title, message, type: 'info', duration: 3000 });
            }
        } catch (e) { /* no-op */ }
    }
    ...
    showSuccessMessage(message) {
        ...
        if (typeof window.officeShowToast === 'function') {
            window.officeShowToast({ title: message, message: '', type: 'success', duration: 4000 });
